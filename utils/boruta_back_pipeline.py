...
class GroupTimeSeriesSplit:
    """Splits data by group labels in time order, sklearn-compatible for outer CV."""
...
class InnerRandomTrainSplit:
    """
    Inner CV: keep the last year as validation, and do K random train/test splits within the earlier years.
    """
    def __init__(self, years: pd.Series, n_splits: int = 3, train_fraction: float = 0.8, random_state: int = 42):
...
# Build pipeline using InnerRandomTrainSplit for inner CV
...
def build_pipeline(
    ...
    *, tune_before_fs=False, tune_after_fs=True,
    ...
    inner_cv = InnerRandomTrainSplit(groups, n_splits=inner_splits,
...
def _run_fold(i, tr, te, X, y, groups, run_args):
    try:
...
def run_parallel(
    ...
    *, groups: pd.Series, estimator_name='randomforest',
    ...
    outer_cv = GroupTimeSeriesSplit(n_splits, groups)
    run_args = dict(
    ...
    tasks = [delayed(_run_fold)(i, tr, te, X, y, groups, run_args)
...
if __name__ == '__main__':
    ...
    groups = pd.Series(np.repeat(np.arange(2015, 2025), 20))
